...
It is possible to store this token in a database using some code like this in the User model:
...
However, the downside of this approach is that each time we want to authenticate a user, we have to query the database which is slow.

### JWT authentication

This is where [JWT Authentication](http://self-issued.info/docs/draft-ietf-oauth-jwt-bearer.html#rfc.section.1) comes in.
...
How are we going to generate a secret key?

> **Note:** We could use the Rails inbuilt secret_key using:

> ```ruby
Rails.application.secrets.secret_key_base
```

> but for the purposes of this lesson we're not.

### Generating a Hashing Key

In order to make this key, we're going to generate a key using Ruby's [Digest](http://ruby-doc.org/stdlib-2.0.0/libdoc/digest/rdoc/Digest.html) module and then store that in our `config/secrets.yml`.

First let's generate a key. Open up irb:
...
Great! **For the minute, copy this into our Auth class temporarily.**

Remember, we want to store sensitive information like this secret as environment variables for a few reasons:
...
There are several ways to implement this in Rails:

- We could use a gem like
	- [Figaro](https://github.com/laserlemon/figaro)
	- [dotenv-rails](https://github.com/bkeepers/dotenv)
- Use an environment variable directly in our class

### Storing our secret securely

We're going to use the [Figaro](https://github.com/laserlemon/figaro) gem for this example. Despite the fact that at the time of writing it's [less popular](https://ruby.libhunt.com/project/figaro/vs/dotenv), the setup is slightly easier.

We need to add the gem to our Gemfile.

```ruby
gem "figaro"
```

Then we need to update our bundle:

```bash
$ bundle
```

Next, we need to install Figaro with:

```bash
$ bundle exec figaro install
```

> **Note:** You might have to run `rbenv rehash` to get this working.

This creates a commented `config/application.yml` file and adds it to your `.gitignore`. This is helpful because we can put all of our environment variables in one file and Figaro will take care of creating environment variables for us.

Next, we want to add this secret to our application.
...
In our `authentications_controller.rb`, we can generate a JWT token and send that in both our register and login actions:
